...
function initReview(QUIZ_DATA){
  ...
  // ---------------------------
  const user = JSON.parse(localStorage.getItem('mudengify_admin_view') || "null");
  const Res = `mudengify_result_${user.user}_${user.Mapel}`;
  const UserAns = `mudengify_answer_${user.user}_${user.Mapel}`;
